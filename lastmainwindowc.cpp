...
bool MainWindow::openSerialPort()
{
    ...
    if (m_serial->open(QIODevice::ReadWrite))
    {
        if (m_serial->setBaudRate(m_settings.value("baud", "-1").toInt(), (QSerialPort::Directions)3) // Was p.baudRate
            ...
            && m_serial->setDataErrorPolicy((QSerialPort::DataErrorPolicy)m_settings.value("datapolicy", "-1").toInt()))
        {
//            while (!m_serial->atEnd())
//                QByteArray data = m_serial->readAll();

            ui->textBox->appendPlainText(tr("Comm Port Connected"));
            //ui->textBox->appendPlainText(tr("Baudrate = %n", QSerialPort::DataBits[(m_serial->DataBits()]));
        }
        else
        {
            m_serial->close();
...
//        ui->textBox->appendPlainText("Token is " + m_settings.value(tokens.at(indx), 0).toString());
        ...
        case 2:
            double currexttemp, currbedtemp;
            m_settings.endGroup();
...
//            ui->textBox->appendPlainText("Ext Temp is " + values.at(m_settings.value("exttempoff", 0).toInt()));
...
//            ui->textBox->appendPlainText("Bed Temp is " + values.at(m_settings.value("bedtempoff", 0).toInt()));
